...
public class BeanInfo
{
	...
	public static String toSimpleString(Object obj) throws IllegalAccessException, InvocationTargetException
	{
		...
		if (methods.length > 0)
        {
            ...
            for (int i = 0; i < methods.length - 1; i++)
                strBuf.append(methods[i].invoke(obj)).append(',');
            strBuf.append(methods[methods.length - 1].invoke(obj));
        }
        strBuf.append('}');
	...
	private BeanInfo(Class<?> clazz, boolean isPrivateAccess)
	{
		...
		getterNames = getMethods.keySet().toArray(new String[getMethods.keySet().size()]);
		setterNames = setMethods.keySet().toArray(new String[setMethods.keySet().size()]);
		fieldNames = fields.keySet().toArray(new String[fields.keySet().size()]);
	}
	...
	private void addMethods(Class<?> cls)
	{
		...
		try
		{
			...
			for (int i = 0; i < propertyDescriptors.length;i++)
			{
				...
				String name = dropCase(getter.getEngineName());
				getMethods.put(name, getter);
				getTypes.put(name, getter.getReturnType());

				Method setter = propertyDescriptors[i].getWriteMethod();
				name = dropCase(setter.getEngineName());
				setMethods.put(name, setter);
				setTypes.put(name, setter.getParameterTypes()[0]);
			}
		}
		catch (IntrospectionException e)
		{
			throw new BeanNotAccessException("failed Method analysis. ", e);
		}
		*/
	}
	...
	{
		return (Field[]) fields.values().toArray(new Field[0]);
	}
	...
	{
		return (Method[]) setMethods.values().toArray(new Method[0]);
	}
	...
	{
		return (Method[]) getMethods.values().toArray(new Method[0]);
	}
