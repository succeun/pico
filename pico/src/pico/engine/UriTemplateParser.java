...
public class UriTemplateParser {
	...
	private static final Pattern TEMPLATE_VALUE_PATTERN = Pattern
			.compile("[^/]+?");
	...
	private static final class StringCharacterIterator implements
			CharacterIterator {
		int pos;
		String s;
	 ...
	 *             if the template is null, an empty string or does not conform
	 *             to a JAX-RS URI template.
	 */
	public UriTemplateParser(String template) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Invalid syntax for the template expression '" + regex
							+ "'", ex);
		}
	}
	 ...
	 * <p>
	 * Any nested capturing groups will be ignored and the the group index will
	 * refer to the top-level capturing groups associated with the templates
	 * variables.
	 * 
	...
	private void processLiteralCharacters() {
		if (literalCharactersBuffer.length() > 0) {
			...
			String s = encodeLiteralCharacters(literalCharactersBuffer
					.toString());
	...
	private void parseName(CharacterIterator ci) {
		...
		try {
			...
			Pattern namePattern = (nameRegexString.length() == 0) ? TEMPLATE_VALUE_PATTERN
			...
			throw new IllegalArgumentException(
					"Invalid syntax for the expression '" + nameRegexString
							+ "' associated with the name '" + name + "'", ex);
		}
	}
	...
	public static UriPattern createUriPattern(UriTemplateParser templateParser) {
		return new UriPattern(templateParser.getPattern(), templateParser.getGroupIndexes());
	}
	
	public static Map<String, String> parseUriPattern(String patternPath, String uri) {
		return parseUriPattern(false, patternPath, uri);
	}
	
	public static Map<String, String> parseUriPattern(boolean isCache, String patternPath, String uri) {
		List<String> names = null;
		UriPattern pattern = null;
		boolean isExist = false;
		if (isCache) {
			ParsedData p = parsedData.get(patternPath);
			if (p != null) {
				names = p.names;
				pattern = p.pattern;
				isExist = true;
		...
		UriTemplateParser templateParser = new UriTemplateParser(prefixWithSlash(patternPath));
		names = templateParser.getNames();
		pattern = new UriPattern(templateParser.getPattern(), templateParser.getGroupIndexes());
		if (isCache && !isExist) {
			parsedData.put(patternPath, new ParsedData(names, pattern));
		}
		
		Map<String, String> values = new HashMap<String, String>();
		if (pattern.match(uri, names, values)) {
			return values;
		} else {
			return null;
		}
	...
	private static Map<String, ParsedData> parsedData = new ConcurrentHashMap<String, ParsedData>(); 
	
	private static class ParsedData {
		public List<String> names = null;
		public UriPattern pattern = null;
		
		public ParsedData(List<String> names, UriPattern pattern) {
			this.names = names;
	...
	public static void main(String[] args) {
		System.out.println(parseUriPattern("/user/{id}{format:(/format/[^/]+?)?}{encoding:(/encoding/[^/]+?)?}", "/user/3/format/pdf/encoding/utf8"));
		System.out.println(parseUriPattern("/user/{id}.{name}", "/user/abc.dec"));
		System.out.println(parseUriPattern("/download/{filename}", "/download/Å×½ºÆ®.txt"));
		System.out.println(parseUriPattern("/user/id={id}&age={age}&name={name}", "/user/id=345&age=12&name=Eun"));
		int max = 1000000;
		StopWatch sw = new StopWatch();
		sw.start();
		for (int i = 0; i < max; i++) {
			parseUriPattern("/user/{id:[0-9]+}", "/user/3456");
		}
		sw.stop();
		...
		sw.start();
		for (int i = 0; i < max; i++) {
			parseUriPattern(true, "/user/{id:[0-9]+}", "/user/3456");
		}
		sw.stop();
		System.out.println(sw.toString());
	}
